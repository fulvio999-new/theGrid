...
MainView {
    ...
    */
    ...
    /* Called from SaveContentsDelegate.qml when the user has selected a saved grid. To show the name of currently opened grid */
             ...
             WebView {
                 ...
                 */
                 onNavigationRequested:{
                    ...
                    // console.log("Url: "+url);
                     var res = url.split("dataToSave=");
                    //  console.log("Value to save: "+res[1]);

                     /* Save grid content as json */
                     //if (UriUtils.url_starts_with(url, 'index.html?dataToSave=')) {
                    //    console.log("MATCH !!!!");
                     //if (UriUtils.url_starts_with(url, 'http://savejson/')) {
                        ...
                        // var urlContent = request.url.toString().replace(/^index\.html?dataToSave=/,'');
                         /* extract from the url the json to save in the DB */
                        // var jsonDecoded = decodeURIComponent(urlContent);
                         var jsonDecoded = decodeURIComponent(res[1]);
                         //console.log("Content to save: "+jsonDecoded);
